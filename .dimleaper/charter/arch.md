...
- **`ssot-schemas/db-schemas/stemrobin.sql` is the single source of truth for the DB tables**
  ...
  from anything reviewable. **It is currently out of date** — it still describes `stemrobin-schema`
  and predates the tables the live schema has grown (`sr_content_ledger`,
  `sr_content_answer_events`, `sr_practice_attempts`, `sr_quiz_attempts`, `sr_recite_attempts`,
  `sr_textbook_mistakes`, `ld_user_emails`) and its column changes (`sr_users` keys on `user_id`,
  not `id`). Bringing it in line with `lemmadeck-schema` is code work and needs its own ticket; until
  then **the live schema is the truth and this file is not to be applied to it**.
...
- The DDL file `ssot-schemas/db-schemas/stemrobin.sql` describes the **retired** `stemrobin-schema`.
  Do not apply it to anything until the ticket that rewrites it lands.
...
6. **Adding a repo-root `package.json`, or a second application** — not without the human's explicit
