...
- `ssot-schemas/db-schemas/lemmadeck.sql` **describes** the live schema (generated from it,
  STEMROBIN-124). It is not a migration and is never applied to the shared database.
- `.agents/skills/lib/content-db.mjs` — the only definition of the content DB's URL resolution
...
1. **Opening a database connection anywhere under `app/` other than `app/src/lib/db.ts`** —
   forbidden outright. A second `postgres` client, or a connection string reaching any file under
   `app/src/routes/` or any `.tsx`, is the crossing this names. (Scoped to `app/` by STEMROBIN-124:
   the content scripts under `.agents/skills/` are node CLIs that have always connected directly —
   entry 6 governs them.)
...
6. **A content script under `.agents/skills/` calling `postgres(` directly** — forbidden outright.
   ...
   `postgres(`; six copied call sites are what made the content pipeline silently outlive a
   database move.
