...
-- ===========================================================================
-- PRACTICE ATTEMPTS + PROGRESS (STEMROBIN-29, batch 0005-progress-and-access)
-- ---------------------------------------------------------------------------
-- sr_practice_attempts — one row per graded practice (课后练习) submission: a
-- learner's SCORE (percent) on a lesson's exercise deck at a point in time. Only
-- the latest TWO attempts per (user_id, lesson_id) are kept — the recording path
-- (app/src/lib/progress.ts recordPracticeAttempt) deletes rows outside the newest
-- two after each insert (human ruling: keep current + previous only). Enforced in
-- code, NOT a trigger. lesson_id is the locale-AGNOSTIC lesson identity: the same
-- lesson in any locale shares one lesson_id, so progress is one point-pair per
-- lesson regardless of language. `score` is a percent in [0,100]; practice-complete
-- (see getProgress) = the LATEST attempt's score >= 80 (can regress). Disposable
-- learner data; cascades away with the learner or the lesson. This block is
-- additive + idempotent (new table only — no ALTER of existing tables).
--
-- NOTE: distinct from the legacy sr_quiz_attempts (open/ended passes over the
-- RELATIONAL sr_questions deck). This table stores JSONB-world practice SCORES.
-- ---------------------------------------------------------------------------
...
-- Latest-attempt lookup + latest-2 pruning both order by (submitted_at desc, id desc)
...
-- Recitation attempts (STEMROBIN-89). Every ladder submission by a logged-in learner
-- is recorded: which level, whether it passed, and whether a hint/answer was used
-- (an "辅助完成" pass is not a real pass). This is deliberately more than the ladder
-- itself needs — the spaced-repetition scheduler in a later batch has to reason over
-- attempt history, and it cannot invent a past that was never recorded.
CREATE TABLE IF NOT EXISTS sr_recite_attempts (
  id          BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  user_id     BIGINT NOT NULL REFERENCES sr_users(user_id) ON DELETE CASCADE,
  lesson_id   TEXT   NOT NULL REFERENCES sr_lessons(id)    ON DELETE CASCADE,
  level       INT    NOT NULL CHECK (level BETWEEN 1 AND 5),
  node_id     TEXT,                                  -- sentence id; null = whole-passage (L5)
  is_correct  BOOLEAN NOT NULL,
  assisted    BOOLEAN NOT NULL DEFAULT false,        -- hint or answer was revealed
  created_at  TIMESTAMPTZ NOT NULL DEFAULT now()
);

CREATE INDEX IF NOT EXISTS sr_recite_attempts_user_lesson_idx
  ON sr_recite_attempts (user_id, lesson_id, created_at);

-- ===========================================================================
-- LEMMADECK ACTIVE DB: DURABLE TEXTBOOK MISTAKES (STEMROBIN-114)
-- ---------------------------------------------------------------------------
-- The application/content database has moved to Supabase schema
-- `lemmadeck-schema`; the legacy Azure section above remains untouched until a
-- boundary migration reconciles the whole SSOT. Apply ONLY this marker-bounded
-- block to LEMMADECK_DATABASE_URL. Unlike disposable answer events, these rows
-- are learner-visible history and survive later correct answers or lesson-row
-- replacement. lesson_id is intentionally not an FK for that reason.
-- BEGIN LEMMADECK TEXTBOOK MISTAKES
CREATE TABLE IF NOT EXISTS "lemmadeck-schema".sr_textbook_mistakes (
  id               BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  user_id          BIGINT NOT NULL
                     REFERENCES "lemmadeck-schema".sr_users(user_id) ON DELETE CASCADE,
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'sr_textbook_mistakes_identity_nonempty'
      AND conrelid = '"lemmadeck-schema".sr_textbook_mistakes'::regclass
  ) THEN
    ALTER TABLE "lemmadeck-schema".sr_textbook_mistakes
      ADD CONSTRAINT sr_textbook_mistakes_identity_nonempty
      CHECK (
        btrim(book_id) <> ''
        AND btrim(lesson_id) <> ''
        AND btrim(exercise_number) <> ''
      );
  END IF;
END
$$;

CREATE INDEX IF NOT EXISTS sr_textbook_mistakes_user_time_idx
  ON "lemmadeck-schema".sr_textbook_mistakes (user_id, occurred_at DESC, id DESC);
-- END LEMMADECK TEXTBOOK MISTAKES

-- ===========================================================================
-- LEMMADECK REGISTRATION EMAILS
-- ---------------------------------------------------------------------------
-- Lightweight registration queue. The public form stores only a normalized
-- email; creating a learner account remains a separate administrative action.
-- BEGIN LEMMADECK USER EMAILS
CREATE TABLE IF NOT EXISTS "lemmadeck-schema".ld_user_emails (
  id          BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  email       TEXT UNIQUE NOT NULL CHECK (btrim(email) <> ''),
  created_at  TIMESTAMPTZ NOT NULL DEFAULT now()
);
-- END LEMMADECK USER EMAILS
