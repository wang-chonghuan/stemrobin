...
6. **练习** (`practice`, the LAST section) — 8–15 items in `<ol class="sr-practice">`. Tag each with a `.sr-ptype` label. Coverage MUST span these categories (math exercise types): **辨认 / 表示 / 操作 / 反推 / 辨错** (识别概念、语言⇄图⇄式互转、基础计算或变形、从结果倒推条件、指出一个常见错误为何错). Provide answers in a **hidden `<div class="sr-answer">…</div>`** per item. Word problems (应用题) are out of scope for now — exclude them.
