...
6. **练习** (structured, authored by **capability 2**) — 8–15 items written to `stemrobin.sr_questions`, one row per item: `type` (**辨认 / 表示 / 操作 / 反推 / 辨错** — coverage MUST span these), `prompt` (KaTeX), `answer_mode: "choice"`, `options` + `correct_index` (at least 3 options, exactly one correct, more than 4 allowed, distractors grounded in the taught misconceptions), and a hidden `answer` (worked solution, never pre-revealed). Word problems (应用题) are out of scope — exclude them. The card-quiz UI renders every item with post-answer feedback + answer reveal.
