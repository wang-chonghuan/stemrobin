...
**`known-gaps.json` is the list of words VOA1500 does not have** — machine-generated by
`scripts/audit-vocab.mjs`, each with a suggested reword. It includes some words you would
never suspect: `where`, `into`, `hello`, `sorry`, `evening`, `something`, `everyone`,
`homework`, `breakfast`, `table`, `chair`. Skim it before writing
...
stops resolving, if one of its 21 must-resolve / 17 must-fail cases regresses, or if the
gate starts admitting non-VOA words:
...
| `known-gaps.json` | the words VOA1500 lacks, with a reword for each — read before writing, regenerate with `audit-vocab.mjs --emit-gaps` |
