...
2. **content** (card-tree) → `sr_lessons.content = { cards: [ { id, num, name, anchor, rev, body[], read_check[] } ] }`. `name` is the section display name (中文名, e.g. `讲解` / `例题`) — a **required** per-card field, rendered as the section label. Each substantial card carries its own read-check (读没读题).
...
4. **Choice-only questions — exactly 4 options (A/B/C/D), single answer.** Both surfaces — each card's **read-check** and every **exercise** — are single-answer choice questions with **exactly four options**: one correct, three distractors. This is a reversible policy (STEMROBIN-25) enforced by `scripts/question-policy.mjs` (`CHOICE_ONLY`) through `check-content.mjs` (read-check) + `check-exercises.mjs` (exercises); the `input`/`work` code paths, the schema `input` mode, and the app's `input` judging are all retained and re-enabled by flipping the flag. Each distractor must be the **same kind and surface form as the correct answer** (a number question's distractors are other numbers; an expression question's are other expressions) and be wrong for a **nameable, common misconception**. Never mix a concrete answer with meta-sentence "误区标签" options, never use filler like "无法确定 / 以上都不对", and never leave the correct option as the only one of its form — that makes the answer guessable at a glance. If you cannot name why a child would pick a distractor, it is not good enough.
