...
Use this guide before reading, writing, or applying `DESIGN.md`.

## Purpose

`DESIGN.md` is the repository's plain-text design-system source for agents. It should describe stable visual identity, layout rules, component patterns, interaction conventions, and content tone clearly enough that future UI work can preserve StemRobin's design without rediscovering it from code.
...
Use these sources when this guide does not answer a new design-documentation question, when changing `DESIGN.guide.md` itself, or when checking whether the current guidance still matches the latest official/spec or common-practice expectations. Routine `DESIGN.md` updates do not require reading these links first when this guide already gives clear direction.
...
- Google Stitch `DESIGN.md` overview and format docs:
