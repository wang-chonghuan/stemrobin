---
name: ld-page2class
description: 把纯扫描的教材 PDF 抽成课程单元——每个小节一份完整课文 + 一组独立编号的题，插图和表格是矢量 SVG，每道题带着自己的图。模型看图认对象，机器吸附像素、跨页装订、全书对账。用于无文字层的老教材数字化。
---
...
4. **TOC 覆盖** —— 目录说这段页码里有几个小节，就该装订出几个小节。
...
产出 `lessons/<小节>/`：`lesson.md`（完整课文）、`exercises.json`（每题独立编号、
...
每个小节两张白底页：`text.html`（课文）+ `exercises.html`（每题独立编号、带自己的图）。
...
11 页 → **4 个小节、55 道题（1–55 连续无缺号）、13 张编号插图 + 1 张无编号表格**。
