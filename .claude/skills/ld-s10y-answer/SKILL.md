...
- **cap1：答案抄录**，忠实保存书后答案页。
- **cap2：lesson 答案生产**，为指定 lesson 的每个 exercise 生成可展示、可判题的答案键并写库。
...
# 按模板生成 page2class/5m/answers.json
...
稳定产物是 `page2class/<book>/answers.json`：
...
# 模型逐题完成 lessons/<id>/answer-keys.json
python3 $K finalize --book 5m \
  --lesson math5-c1-s1-n5 --lesson math5-c1-s1-n6 --lesson math5-c1-s1-n7

node .claude/skills/ld-s10y-answer/tools/publish.mjs page2class/5m \
  --lesson math5-c1-s1-n5 --lesson math5-c1-s1-n6 --lesson math5-c1-s1-n7
```
...
2. `page2class/<book>/answers.json` 有书后答案时优先参考并保留为 `bookRaw`。
...
```text
page2class/<book>/lessons/<lesson-id>/answer-keys.json
```
...
`finalize` 和真实 `publish`。`--dry` 只能检查，不能作为交付终点。
