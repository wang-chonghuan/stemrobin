# n-im intent — STEMROBIN-31 · login-gate
...
## Scope
应用**所有页面都需登录**才能查看：未登录用户访问任意页面（首页/目录/课程/练习/进度等）一律被引导到登录页；登录后正常访问。**不提供注册**，仅现有账号可登录。

## Constraints
- 除登录页外全部页面需登录；无注册入口。
- 沿用现有 HMAC session 机制；不动 `sr_users`；登录后原有体验不变。

## Acceptance criteria
- 未登录状态下访问任意应用页面都会被要求先登录（看不到内容）。
- 用现有账号登录后可正常访问全部页面。
- 页面上没有"注册/创建账号"入口。
