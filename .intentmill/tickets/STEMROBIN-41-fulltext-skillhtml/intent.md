# n-im intent — STEMROBIN-41 · 全文速览改用 skill 渲染 html 对齐 PDF

> prodfarm cap9 组合。Batch 0007 · Seed STEMROBIN-39（full delegation）。已含 40(存量 sr_lessons.html 已从 JSONB 重渲为美观版：sr-sec-num 编号+中文名+样式化练习区，= PDF 同款)+42(卡片标题编号+色)。
> 依赖 STEMROBIN-40 已交付：sr_lessons.html 现为最新美观版(编号+名+练习区)。
...
## Scope
全文速览改用 **skill 渲染的课文 html**（存量 `sr_lessons.html`，与 PDF 同款渲染器）呈现整篇，取代当前自建的简陋 `buildFullTextHtml`——使全文速览达到 **PDF 品质**：section 前带序号、课后题样式美观。课后题仍**只显示、不判分、不计进度**（该 html 的练习区为 prompt-only、无 KEY）。

## Constraints
- 依赖存量 html 已重渲为美观版（本 seed 另一单）。
- 速览里课后题只显示、不作答判分、不推进进度。
- 公式正常渲染、移动端不横向溢出；沿用登录门禁；卡片精读流不变。

## Acceptance criteria
- 切到全文速览，呈现与 PDF 一致的整篇课文（section 前带序号、课后题样式美观），不再是简陋列表。
- 速览里的课后题不可作答、不计进度。
- 公式渲染正常、手机窄屏无横向溢出。
