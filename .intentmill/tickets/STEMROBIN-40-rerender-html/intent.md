# n-im intent — STEMROBIN-40 · rerender-html

> prodfarm cap9 组合。Batch 0007-reading-polish · Seed STEMROBIN-39（full delegation）。已含 batch 0004/0005/0006。
...
- Origin: seed
- Seed: [STEMROBIN-39](https://app.plane.so/intentmill/projects/556c9df8-f5ac-435a-8abd-53bc522dd8a7/work-items/4bdcf634-4b4f-43cd-977f-4594ada21ec3)

## Scope
打磨 skill 渲染并重渲存量：`render-lesson.mjs` 若 section 编号/名/配色或练习区样式有可提升处则打磨（保证每 section 前带序号 + 中文名、课后题区样式美观、对比 PDF 一致）；并从 JSONB **重渲 16 课的 `sr_lessons.html`/`pdf` 缓存**（STEMROBIN-34 遗留未做），使存量为最新美观版（含 section 编号+名）。

## Constraints
- 走 skill 确定性渲染、幂等；不改课文正文/JSONB content 数据结构（仅重渲派生 html/pdf）。
- 答案保密：渲染出的 html 练习区只含 prompt/选项、无答案 KEY。
- 不动 `sr_users`；不引新依赖。

## Acceptance criteria
- 重渲后 16 课的 `sr_lessons.html` 每个 section 都带序号 + 中文名，练习区样式美观（与 PDF 呈现一致）。
- 用生成器新产的样例课渲染出的 html 也如此。
- 渲染出的 html 不含答案 KEY（correct_index/accept/answer）。
