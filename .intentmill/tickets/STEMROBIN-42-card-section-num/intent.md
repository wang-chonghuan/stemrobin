# n-im intent — STEMROBIN-42 · card-section-num
...
## Scope
现象：卡片精读的 section 标题只有中文名、无编号，且全黑不明显。期望：section 标题显示**编号 + 中文名**（如 "1 为什么学这个"），并换成**明显的颜色**（不再纯黑）。

## Constraints
- 编号取自 `card.num`、名取自 `card.name`；用现有 `--sr-*` token 的强调色（teal-blue/green），不引新色相。
- 中/英行为一致；仅 `app/`（card-reader + css）；不引新依赖。

## Acceptance criteria
- 卡片精读的 section 标题形如 "1 为什么学这个"（带序号）。
- section 标题颜色明显、非纯黑。
- 课文标题仍正常显示。
