# n-im intent — STEMROBIN-36 · login-logout
...
## Scope
提供**登录页**与**登出**入口/页：未登录跳登录页、可用现有账号登录进入应用；已登录时有可见的登出操作，登出后回到登录页。暂不需要注册。

## Constraints
- 无注册入口；用户后台手动加（只测当前账号）。
- 沿用现有 HMAC session；不动 `sr_users`；登录页是干净登录界面、不暴露受保护内容。

## Acceptance criteria
- 用现有账号在登录页登录后进入应用。
- 应用内有可见登出操作，点击后退出并回到登录页。
- 页面无"注册/创建账号"入口；登出后访问受保护页被要求重新登录。
