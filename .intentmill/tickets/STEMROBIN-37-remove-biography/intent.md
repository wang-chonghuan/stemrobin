# n-im intent — STEMROBIN-37 · remove-biography
...
## Scope
现象：名人传记仍在 app 里（侧边栏传记区、首页传记相关、story 路由/组件/lib）。期望：从 **app 代码**彻底删除名人传记（侧边栏 + 首页 + story 路由/组件/lib 全删）。**保留** `sr-story` 生成 skill 与传记 DB 数据。

## Constraints
- 只删 app 代码里的传记 UI/路由/lib；**保留** `.agents/skills/sr-story` 与 `sr_stories`/`sr_story_*` 数据。
- 不动 `sr_users`；删后 app 构建与现有数学功能不受影响。

## Acceptance criteria
- 应用侧边栏与首页无任何名人传记入口/区块。
- 无 story 路由可达（传记页不可访问）。
- `sr-story` skill 与传记 DB 数据仍在；app 构建通过、数学功能正常。
