# n-im intent — STEMROBIN-30 · 练习按次计分与首页真进度条

> prodfarm cap9 组合。Batch 0005-progress-and-access · Seed STEMROBIN-26（full delegation）· plane。
> 依赖已交付：STEMROBIN-29 提供 app/src/lib/progress.ts（recordPracticeAttempt(lessonId,score) / getProgress() → per-lesson readingComplete+practiceComplete + totalPoints(2×课数=32)+completedPoints）+ sr_practice_attempts（每用户每课留最近2次）。
...
## Scope
① 课后练习改为**按次计分**：学习者完成一次练习作答后得到**本次分数**（正确率），记为一次 attempt；② 首页显示**真实进度条**——按现有所有课的总点数（2 × 课数）与用户已完成点数，取代当前的假进度；进度随最近练习 attempt 升降。

## Constraints
- 练习仍为纯选择题、服务端判分；一次 attempt = 一遍练习作答，分数 = 正确率。
- 进度条数据来自练习 attempt 与课文完成的进度读取能力（依赖 STEMROBIN 的进度数据模型单）。
- 课文点走完卡片即完成（不回退）；练习点随最近 attempt 可回退。
- 沿用答案保密与登录门禁。

## Acceptance criteria
- 完成一次课后练习后学习者看到本次分数；该分数 ≥80% 使该课"练习进度"完成，若之后再做一次 <80% 则该练习点回退为未完成。
- 首页进度条反映"用户已完成点数 / 总点数（2 × 课数）"的真实值，不再是固定假数字。
- 走完某课全部卡片后，该课"课文进度"点计入首页进度条。
