...
## Scope
现象：卡片 read-check 的题干/选项里的数学公式显示为**原始 LaTeX 源码**（如 `$\dfrac{x}{2}$`），未被 KaTeX 渲染成数学符号。期望：read-check 的题干与选项公式正确渲染为数学形式，与卡片正文、课后练习抽屉一致。顺带抽查 STEMROBIN-25 自动转换出的 read-check 内容（题干/选项/正确项）是否正确，修正发现的明显错误。

## Constraints
- 只修渲染路径与（如发现）个别错题内容；不改题目模式（read-check 仍为纯选择题）。
- 答案保密不破：正确项等 KEY 不入初次浏览器 payload。
- 不动 `sr_users`；答题事件可弃。

## Acceptance criteria
- 打开含公式的 read-check（如去分母/通分类题），题干与选项的数学符号以渲染后的数学形式显示，页面上看不到原始 `$...$` 源码。
- 卡片正文、read-check、课后练习三处的公式渲染表现一致。
- read-check 提交前的初次数据不含正确答案。
