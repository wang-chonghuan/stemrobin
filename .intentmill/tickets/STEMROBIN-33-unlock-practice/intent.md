# n-im intent — STEMROBIN-33 · unlock-practice
...
## Scope
现象：课后题（练习 deck）被"读完全部卡片才解锁"锁住，未读完不能打开。期望：课后题**随时可打开作答**，不因未读完课文而被锁。

## Constraints
- 只解锁练习入口；不改练习判分/题型（仍纯选择题、服务端判分）。
- 课文进度仍只由卡片精读拿到、练习进度仍按最近 attempt ≥80%（两点独立、互不影响）。
- 不动 `sr_users`；答案保密不破。

## Acceptance criteria
- 打开一篇课，未读完卡片也能打开课后题并作答。
- 课后题作答判分正常。
- 只做课后题、不走卡片精读，不会使该课"课文进度"完成。
