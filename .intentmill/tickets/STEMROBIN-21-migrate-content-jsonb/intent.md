# n-im intent — STEMROBIN-21 · 迁移既有数学内容进 JSONB SSOT

> prodfarm cap9 组合：工单全文 + live charter + evodocs。self-contained。Batch 0004 · Seed STEMROBIN-18（full delegation）· plane。
> 依赖已交付：T1(STEMROBIN-19) DB JSONB 结构；T2(STEMROBIN-20) 生成器 JSONB-first（.agents/skills/sr-math-lesson 的 render-lesson.mjs/save-lesson.mjs/save-ledger.mjs/check-content.mjs 等可复用）。

---
## 一、交付工单 STEMROBIN-21（描述全文）
...
## Scope
把既有内容迁入 JSONB SSOT，**不重新生成课文**：现有 stage 概念账本从本地文件迁入 DB；16 篇已存数学课的课文 HTML 解析为**有序带编号的卡片树 JSONB**、既有练习迁为练习 deck JSONB；为每张实质卡片补齐 read-check。复用生成器的渲染/校验能力。课文若结构不够干净，为迁干净做**最小必要改动**，每篇产出改动 diff 与原文快照。

## Constraints
- 只迁 `zh` 源；其它语言属后续翻译。
- 迁移以课文既有语义分节为卡片边界；排除由练习注入的非课文区。
- 允许为切干净做最小必要改动，但不改教学内容/公式/例子实质；每篇留 diff + 快照。
- 迁移幂等（重跑覆盖非追加）；可弃被替换内容相关答题事件；不得动 `sr_users` 凭据行。
- 迁移后本地 `resources/content/math-ledger/*.json` 不再是权威源。
- 经确定性脚本落库，不手写数据行。

## Acceptance criteria
- 迁移后，16 篇课每篇在 DB 中都有其卡片树课文 JSONB（有序带编号、不含练习注入区）、练习 deck JSONB，且每张实质卡片挂有 ≥2 道 read-check。
- 现有各 stage 概念账本均可从 DB 取到；不依赖本地 ledger 文件也能渲染这些课。
- 每篇迁移可取到"改动 diff + 原文快照"，据此确认课文实质未变或仅切分性微调。
