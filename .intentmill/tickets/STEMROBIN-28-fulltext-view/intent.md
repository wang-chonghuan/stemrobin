# n-im intent — STEMROBIN-28 · fulltext-view
...
## Scope
学习者在课程页可切换**"逐卡精读"与"全文速览"**两种阅读方式。全文速览一次性显示整篇课文的**全部内容**（= PDF 同源内容），**不带卡片 read-check、不设门禁**，随时可看（无需答完题）。

## Constraints
- 全文速览不记录 read-check、**不推进课文进度**（只看全文不算进度）。
- 逐卡精读模式保持现状（逐卡 + read-check 软门禁）。
- 公式/图正常渲染、移动端不横向溢出。
- 沿用登录门禁（本 seed 另有专单）。

## Acceptance criteria
- 课程页可切换到"全文速览"，一次看到整篇课文的全部内容，且不出现卡片小练习/门禁。
- 从速览切回逐卡精读，精读流照常工作。
- 仅用速览看完全文，不会使该课"课文进度"变为完成。
